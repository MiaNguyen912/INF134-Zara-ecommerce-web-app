...
export default function Carousel({ images, className, interval = 5000 }: CarouselProps) {
  ...
  return (
    <div className={`${className} h-screen`}>
      ...
      {/* Navigation Dots */}
      {images.length > 1 && (
        <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col space-y-2 z-10">
          {images.map((_, index) => (
            <button
              key={index}
